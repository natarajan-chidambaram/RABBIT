...
def CreatingRepository(df_events, list_event_id_covered, df_all_activities):
    ...
    create_repository = (
        df_events[event_general + ["ref_type"]]
        .assign(activity="Creating repository")
        .rename(columns={"login": "contributor", "created_at": "date"})
        .assign(
            date=lambda d: d.date.dt.tz_localize(tz="UTC").map(lambda x: x.isoformat())
        )
        .drop_duplicates()
        .sort_values("date")
    )
    ...
    df_all_activities = pd.concat(
        [df_all_activities, create_repository.drop_duplicates()[general]]
    )
...
def PublishingRelease(df_events, list_event_id_covered, df_all_activities):
    ...
    if df_events.columns.isin(["ref"]).any():
        df_rel_cr = (
            ...
            .assign(
                cr_event_type=lambda d: np.where(
                    (
                        (d.event_type == "ReleaseEvent")
                        & (d.next_event == "CreateEvent")
                        & (d.login == d.next_login)
                        & (d.repository == d.next_repository)
        ...
        if df_rel_cr.shape[0] > 0:
            df_rel_cr = df_rel_cr.rename(
                columns={
                    "next_event_created_at": "cr_created_at",
                    "next_ref": "cr_ref",
        ...
        df_cr_rel = (
            ...
            .assign(
                cr_event_type=lambda d: np.where(
                    (
                        (d.event_type == "ReleaseEvent")
                        & (d.prev_event == "CreateEvent")
                        & (d.login == d.prev_login)
                        & (d.repository == d.prev_repository)
        ...
        if df_cr_rel.shape[0] > 0:
            df_cr_rel = df_cr_rel.rename(
                columns={
                    "prev_event_created_at": "cr_created_at",
                    "prev_ref": "cr_ref",
        ...
        df_release_with_create = pd.concat([df_rel_cr, df_cr_rel]).drop_duplicates(
            "event_id"
        )

        if df_release_with_create.shape[0] > 0:
            df_release_with_create = df_release_with_create[
                (df_release_with_create["time_diff"] <= UPPER_TIME_THRESHOLD)
            ...
            list_event_id_covered.extend(
                df_release_with_create["cr_event_id"].to_list()
            )

            df_release_with_create_proc = (
                df_release_with_create.rename(
                    columns={"login": "contributor", "created_at": "date"}
                )
                .assign(activity="Publishing a release")
                .assign(
                    date=lambda d: d.date.dt.tz_localize(tz="UTC").map(
            ...
            df_all_activities = pd.concat(
                [
                    df_all_activities,
                    df_release_with_create_proc.drop_duplicates()[general],
                ]
            )
    ...
    """
    df_release_without_create = df_events.query(
        'event_type == "ReleaseEvent" and event_id not in @list_event_id_covered'
    )[event_general + ["release_node_id"]].sort_values(
        ["login", "repository", "created_at"]
    )
    ...
    if df_release_without_create.shape[0] > 0:
        ...
        df_release_without_create_proc = (
            df_release_without_create.rename(
                columns={
                    "login": "contributor",
                    "created_at": "date",
        ...
        df_release_without_create_proc = df_release_without_create_proc.rename(
            columns={"GH_node": "release_GH_node"}
        )
        df_all_activities = pd.concat(
            [
                df_all_activities,
                df_release_without_create_proc.drop_duplicates()[general],
            ]
        )
...
def MakingRepositoryPublic(df_events, list_event_id_covered, df_all_activities):
    ...
    df_all_activities = pd.concat(
        [df_all_activities, public_event.drop_duplicates()[general]]
    )
...
def AddingCollaborator(df_events, list_event_id_covered, df_all_activities):
    ...
    df_all_activities = pd.concat(
        [df_all_activities, member_event.drop_duplicates()[general]]
    )
...
def ForkingRepository(df_events, list_event_id_covered, df_all_activities):
    ...
    df_all_activities = pd.concat(
        [df_all_activities, forking_repository.drop_duplicates()[general]]
    )
...
def StarringRepository(df_events, list_event_id_covered, df_all_activities):
    ...
    df_all_activities = pd.concat(
        [df_all_activities, starring_repository.drop_duplicates()[general]]
    )
...
def EditingWikiPage(df_events, list_event_id_covered, df_all_activities):
    ...
    df_all_activities = pd.concat(
        [df_all_activities, df_wiki_page.drop_duplicates()[general]]
    )
...
def TransferingIssue(df_events, list_event_id_covered, df_all_activities):
    ...
    df_transferring_issue = (
        df_events[event_general + ["issue_node_id"]]
        .rename(columns={"login": "contributor", "created_at": "date"})
...
def ClosingIssue(df_events, list_event_id_covered, df_all_activities):
    ...
    """
    df_closing_issue_or_PR_comment = df_events.drop_duplicates()[
        event_general + ["issue_number", "state", "issue_node_id"]
    ].sort_values(["login", "repository", "created_at"])

    df_closing_issue_and_comment_1 = df_closing_issue_or_PR_comment[
        df_closing_issue_or_PR_comment["issue_number"].notnull()
    ]

    df_closing_issue_before_comment = (
        df_closing_issue_and_comment_1.assign(
            next_event_id=lambda d: d.event_id.shift(-1)
        )
        .assign(next_event=lambda d: d.event_type.shift(-1))
        .assign(next_login=lambda d: d.login.shift(-1))
        .assign(next_repository=lambda d: d.repository.shift(-1))
        ...
        .assign(next_event_created_at=lambda d: d.created_at.shift(-1))
        .assign(
            comm_event_type=lambda d: np.where(
                (
                    (d.event_type == "IssuesEvent")
                    & (d.next_event == "IssueCommentEvent")
                    & (d.login == d.next_login)
                    & (d.repository == d.next_repository)
                    & (d.issue_number == d.next_issue_number)
                    & (d.state == d.next_issue_state)
                ),
                d.next_event,
                np.nan,
            )
    ...
    if df_closing_issue_before_comment.shape[0] > 0:
        df_closing_issue_before_comment = df_closing_issue_before_comment.rename(
            columns={
                "next_event_created_at": "comm_created_at",
                "next_repository": "comm_repository",
                "next_login": "comm_login",
    ...
    df_closing_issue_after_comment = (
        df_closing_issue_and_comment_1.assign(
            prev_event_id=lambda d: d.event_id.shift(1)
        )
        .assign(prev_event=lambda d: d.event_type.shift(1))
        .assign(prev_login=lambda d: d.login.shift(1))
        .assign(prev_repository=lambda d: d.repository.shift(1))
        ...
        .assign(
            comm_event_type=lambda d: np.where(
                (
                    (d.event_type == "IssuesEvent")
                    & (d.prev_event == "IssueCommentEvent")
                    & (d.login == d.prev_login)
                    & (d.repository == d.prev_repository)
                    & (d.issue_number == d.prev_issue_number)
                    & (d.state == d.prev_issue_state)
                ),
                d.prev_event,
                np.nan,
            )
    ...
    if df_closing_issue_after_comment.shape[0] > 0:
        df_closing_issue_after_comment = df_closing_issue_after_comment.rename(
            columns={
                "prev_event_created_at": "comm_created_at",
                "prev_repository": "comm_repository",
                "prev_login": "comm_login",
    ...
    df_closing_issue_with_comment = pd.concat(
        [df_closing_issue_before_comment, df_closing_issue_after_comment]
    ).drop_duplicates()

    if df_closing_issue_with_comment.shape[0] > 0:
        df_closing_issue_with_comment = df_closing_issue_with_comment[
            (df_closing_issue_with_comment["time_diff"] <= UPPER_TIME_THRESHOLD)
            & (df_closing_issue_with_comment["time_diff"] >= LOWER_TIME_THRESHOLD)
        ]

        list_event_id_covered.extend(
            df_closing_issue_with_comment["event_id"].to_list()
        )
        list_event_id_covered.extend(
            df_closing_issue_with_comment["comm_event_id"].to_list()
        )

        df_closing_issue_with_comment_proc = (
            df_closing_issue_with_comment.rename(
                columns={"login": "contributor", "created_at": "date"}
            )
            .assign(
                date=lambda d: d.date.dt.tz_localize(tz="UTC").map(
            ...
            .assign(activity="Closing issue")[general + ["issue_node_id"]]
            .drop_duplicates()
            .sort_values("date")
        )

        df_all_activities = merge_issue_related(
            df_all_activities, df_closing_issue_with_comment_proc
        )

    """
    Obtain the closed issue events that did not invovle any commeting activity while closing
    """
    df_closing_issue_without_comment = df_events.query(
        'event_type == "IssuesEvent" and action == "closed" and event_id not in @list_event_id_covered'
    )[event_general + ["issue_closed_at", "issue_node_id"]]
    if df_closing_issue_without_comment.shape[0] > 0:
        list_event_id_covered.extend(
            df_closing_issue_without_comment["event_id"].to_list()
        )

        df_closing_issue_without_comment_proc = (
            df_closing_issue_without_comment.rename(
                columns={"login": "contributor", "created_at": "date"}
            )
            .assign(
                date=lambda d: d.date.dt.tz_localize(tz="UTC").map(
            ...
            .assign(
                closed_at=lambda d: d.issue_closed_at.dt.tz_localize(tz="UTC").map(
                    lambda x: x.isoformat()
                )
            )
            .assign(activity="Closing issue")[general + ["issue_node_id", "closed_at"]]
            .drop_duplicates()
        )

        df_all_activities = merge_issue_related(
            df_all_activities, df_closing_issue_without_comment_proc
        )
...
def ReopeningIssue(df_events, list_event_id_covered, df_all_activities):
    ...
    df_reopening_issue_or_PR_comment = df_events.drop_duplicates()[
        event_general + ["issue_number", "issue_node_id"]
    ].sort_values(["login", "repository", "created_at"])

    df_reopening_issue_and_comment_1 = df_reopening_issue_or_PR_comment[
        df_reopening_issue_or_PR_comment["issue_number"].notnull()
    ]

    df_reopening_issue_before_comment = (
        df_reopening_issue_and_comment_1.assign(
            next_event_id=lambda d: d.event_id.shift(-1)
        )
        .assign(next_event=lambda d: d.event_type.shift(-1))
        .assign(next_login=lambda d: d.login.shift(-1))
    ...
    if df_reopening_issue_before_comment.shape[0] > 0:
        df_reopening_issue_before_comment = df_reopening_issue_before_comment.rename(
            columns={
                "next_event_created_at": "comm_created_at",
                "next_repository": "comm_repository",
                "next_login": "comm_login",
    ...
    if df_reopening_issue_after_comment.shape[0] > 0:
        df_reopening_issue_after_comment = df_reopening_issue_after_comment.rename(
            columns={
                "prev_event_created_at": "comm_created_at",
                "prev_repository": "comm_repository",
                "prev_login": "comm_login",
    ...
    df_reopening_issue_with_comment = pd.concat(
        [df_reopening_issue_before_comment, df_reopening_issue_after_comment]
    ).drop_duplicates()

    if df_reopening_issue_with_comment.shape[0] > 0:
        df_reopening_issue_with_comment = df_reopening_issue_with_comment[
            (df_reopening_issue_with_comment["time_diff"] <= UPPER_TIME_THRESHOLD)
            & (df_reopening_issue_with_comment["time_diff"] >= LOWER_TIME_THRESHOLD)
        ]
        list_event_id_covered.extend(
            df_reopening_issue_with_comment["event_id"].to_list()
        )
        list_event_id_covered.extend(
            df_reopening_issue_with_comment["comm_event_id"].to_list()
        )

        df_reopening_issue_with_comment_proc = (
            df_reopening_issue_with_comment.rename(
                columns={"login": "contributor", "created_at": "date"}
            )
            .assign(
                date=lambda d: d.date.dt.tz_localize(tz="UTC").map(
            ...
            .assign(activity="Reopening issue")[general + ["issue_node_id"]]
            .drop_duplicates()
            .sort_values("date")
        )
        df_all_activities = merge_issue_related(
            df_all_activities, df_reopening_issue_with_comment_proc
        )

    """
    Obtain the reopenend issue events that did not invovle any commeting activity while closing
    """
    df_reopening_issue_without_comment = df_events.query(
        'event_type == "IssuesEvent" and action == "reopened" and event_id not in @list_event_id_covered'
    )[event_general + ["issue_node_id"]]

    if df_reopening_issue_without_comment.shape[0] > 0:
        list_event_id_covered.extend(
            df_reopening_issue_without_comment["event_id"].to_list()
        )

        df_reopening_issue_without_comment_proc = (
            df_reopening_issue_without_comment.rename(
                columns={"login": "contributor", "created_at": "date"}
            )
            .assign(
                date=lambda d: d.date.dt.tz_localize(tz="UTC").map(
        ...
        df_all_activities = merge_issue_related(
            df_all_activities, df_reopening_issue_without_comment_proc
        )
...
def OpeningPullRequest(df_events, list_event_id_covered, df_all_activities):
    ...
    df_opening_pullrequest = (
        df_events[event_general + ["PR_node_id"]]
        .rename(columns={"login": "contributor", "created_at": "date"})
        .assign(
            date=lambda d: d.date.dt.tz_localize(tz="UTC").map(lambda x: x.isoformat())
        )
...
def ReopeningPullRequest(df_events, list_event_id_covered, df_all_activities):
    ...
    df_reopening_pr_and_comment_1 = df_reopening_issue_or_PR_comment[
        df_reopening_issue_or_PR_comment["PR_number"].notnull()
    ]

    df_reopening_pr_before_comment = (
        df_reopening_pr_and_comment_1.assign(
            next_event_id=lambda d: d.event_id.shift(-1)
        )
        .assign(next_event=lambda d: d.event_type.shift(-1))
        .assign(next_login=lambda d: d.login.shift(-1))
        ...
        .query(
            'event_type == "PullRequestEvent" and comm_event_type == "IssueCommentEvent"'
        )
    )
    if df_reopening_pr_before_comment.shape[0] > 0:
        df_reopening_pr_before_comment = df_reopening_pr_before_comment.rename(
            columns={
                "next_event_created_at": "comm_created_at",
                "next_repository": "comm_repository",
                "next_login": "comm_login",
    ...
    if df_reopening_pr_after_comment.shape[0] > 0:
        df_reopening_pr_after_comment = df_reopening_pr_after_comment.rename(
            columns={
                "prev_event_created_at": "comm_created_at",
                "prev_repository": "comm_repository",
                "prev_login": "comm_login",
    ...
    df_reopening_pr_with_comment = pd.concat(
        [df_reopening_pr_before_comment, df_reopening_pr_after_comment]
    ).drop_duplicates()
    ...
    if df_reopening_pr_with_comment.shape[0] > 0:
        df_reopening_pr_with_comment = df_reopening_pr_with_comment[
            (df_reopening_pr_with_comment["time_diff"] <= UPPER_TIME_THRESHOLD)
        ...
        list_event_id_covered.extend(
            df_reopening_pr_with_comment["comm_event_id"].to_list()
        )

        df_reopening_pr_with_comment_proc = (
            df_reopening_pr_with_comment.rename(
                columns={"login": "contributor", "created_at": "date"}
            )
            .assign(
                date=lambda d: d.date.dt.tz_localize(tz="UTC").map(
            ...
            .assign(pr_node_id=lambda d: d.PR_node_id)[general + ["pr_node_id"]]
            .drop_duplicates()
            .sort_values("date")
        )

        df_all_activities = merge_pr_related(
            df_all_activities, df_reopening_pr_with_comment_proc
        )
    ...
    df_reopening_pr_without_comment = df_events.query(
        'event_type == "PullRequestEvent" and action == "reopened" and event_id not in @list_event_id_covered'
    )[event_general + ["PR_node_id"]]
    if df_reopening_pr_without_comment.shape[0] > 0:
        df_reopening_pr_without_comment = (
            df_reopening_pr_without_comment.rename(
                columns={"login": "contributor", "created_at": "date"}
            )
            .assign(
                date=lambda d: d.date.dt.tz_localize(tz="UTC").map(
        ...
        list_event_id_covered.extend(
            df_reopening_pr_without_comment["event_id"].to_list()
        )

        df_all_activities = merge_pr_related(
            df_all_activities, df_reopening_pr_without_comment
        )
...
def ClosingPullRequest(df_events, list_event_id_covered, df_all_activities):
    ...
    df_pr_merge = (
        df_events.query(
            'event_type == "PullRequestEvent" and action == "closed" and merged == True'
        )
        .dropna(axis=1, how="all")
        .drop_duplicates("event_id")
    )
    df_push_events = (
        df_events.query('event_type == "PushEvent"')
        .dropna(axis=1, how="all")
        .drop_duplicates("event_id")
    )
    if df_pr_merge.shape[0] > 0 and df_push_events.shape[0] > 0:
        df_pr_merge_push = pd.merge(
            df_pr_merge,
            df_push_events,
            left_on=["login", "repository"],
            right_on=["login", "repository"],
        )
        ...
        pr_close_with_push = df_pr_merge_push[
            (df_pr_merge_push["time_diff"] <= UPPER_TIME_THRESHOLD)
        ...
        df_pr_close_with_push_proc = (
            ...
            .assign(pr_node_id=lambda d: d.PR_node_id)
            .assign(comment_node_id=None)[general + ["pr_node_id", "comment_node_id"]]
            .drop_duplicates()
            .sort_values("date")
        )

        df_all_activities = merge_pr_related(
            df_all_activities, df_pr_close_with_push_proc
        )

    """
    Obtain PR events that are closed and merged, but dont have an associated push within the threshold
    """
    df_pr_close_merge_no_push = df_events.query(
        'event_type == "PullRequestEvent" and action == "closed" and merged == True and event_id not in @list_event_id_covered'
    )[event_general + ["PR_node_id"]].rename(
        columns={
    ...
    if df_pr_close_merge_no_push.shape[0] > 0:
        ...
        df_pr_close_merge_no_push_proc = (
            df_pr_close_merge_no_push.assign(
                date=lambda d: d.date.dt.tz_localize(tz="UTC").map(
                    lambda x: x.isoformat()
                )
            )
        ...
        df_all_activities = merge_pr_related(
            df_all_activities, df_pr_close_merge_no_push_proc
        )
    ...
    if (
        df_events.event_type.isin(["PullRequestEvent"]).any()
        and df_events.event_type.isin(["IssueCommentEvent"]).any()
    ):
        df_closing_issue_or_PR_comment = (
            df_events.query(
                '(event_type == "PullRequestEvent" and action == "closed" and merged == False) or event_type == "IssueCommentEvent"'
            )
            .drop_duplicates()[
        ...
        df_closing_pr_and_comment_1 = df_closing_issue_or_PR_comment[
            df_closing_issue_or_PR_comment["PR_number"].notnull()
        ]

        df_closing_pr_before_comment = (
            df_closing_pr_and_comment_1.assign(
                next_event_id=lambda d: d.event_id.shift(-1)
            )
            .assign(next_event=lambda d: d.event_type.shift(-1))
            .assign(next_login=lambda d: d.login.shift(-1))
        ...
        if df_closing_pr_before_comment.shape[0] > 0:
            df_closing_pr_before_comment = df_closing_pr_before_comment.rename(
                columns={
                    "next_event_created_at": "comm_created_at",
                    "next_repository": "comm_repository",
                    "next_login": "comm_login",
        ...
        if df_closing_pr_after_comment.shape[0] > 0:
            df_closing_pr_after_comment = df_closing_pr_after_comment.rename(
                columns={
                    "prev_event_created_at": "comm_created_at",
                    "prev_repository": "comm_repository",
                    "prev_login": "comm_login",
        ...
        df_closing_pr_with_comment = pd.concat(
            [df_closing_pr_before_comment, df_closing_pr_after_comment]
        ).drop_duplicates()

        if df_closing_pr_with_comment.shape[0] > 0:
            list_event_id_covered.extend(
                df_closing_pr_with_comment["event_id"].to_list()
            )
            list_event_id_covered.extend(
                df_closing_pr_with_comment["comm_event_id"].to_list()
            )

            df_closing_pr_with_comment_proc = (
                df_closing_pr_with_comment.rename(
                    columns={"login": "contributor", "created_at": "date"}
                )
                .assign(
                    date=lambda d: d.date.dt.tz_localize(tz="UTC").map(
            ...
            df_all_activities = merge_pr_related(
                df_all_activities, df_closing_pr_with_comment_proc
            )

    df_pr_close_no_merge_without_comment = df_events.query(
        'event_type == "PullRequestEvent" and action == "closed" and merged == False and \
                event_id not in @list_event_id_covered'
    )[event_general + ["PR_node_id"]]

    if df_pr_close_no_merge_without_comment.shape[0] > 0:
        list_event_id_covered.extend(
            df_pr_close_no_merge_without_comment["event_id"].to_list()
        )

        pr_close_without_comment = []
        pr_close_without_comment.extend(
            df_pr_close_no_merge_without_comment["event_id"].to_list()
        )

        df_pr_close_no_merge_without_comment_proc = (
            df_pr_close_no_merge_without_comment.rename(
                columns={"login": "contributor", "created_at": "date"}
            )
            .assign(
                date=lambda d: d.date.dt.tz_localize(tz="UTC").map(
        ...
        df_all_activities = merge_pr_related(
            df_all_activities, df_pr_close_no_merge_without_comment_proc
        )
...
def CommentingPullRequest(df_events, list_event_id_covered, df_all_activities):
    ...
    df_pr_commenting = (
        df_events[event_general + ["comment_node_id"]]
        .sort_values(["login", "repository", "created_at"])
        .rename(
            columns={
                "login": "contributor",
    ...
    df_all_activities = pd.concat(
        [df_all_activities, df_pr_commenting.drop_duplicates()[general]]
    )
...
def CommentingPullRequestChanges(df_events, list_event_id_covered, df_all_activities):
    ...
    if df_events.columns.isin(["comment_node_id"]).any():
        df_pr_review_commenting = df_events[event_general + ["comment_node_id"]]
    elif df_events.columns.isin(["review_node_id"]).any():
        df_pr_review_commenting = df_events[event_general + ["review_node_id"]]
    df_pr_review_commenting = (
        df_pr_review_commenting.rename(
            columns={"login": "contributor", "created_at": "date"}
        )
    ...
    if (
        len(
            df_pr_review_commenting.query(
                'event_type == "PullRequestReviewCommentEvent"'
            )
        )
        > 0
    ):
        df_pr_review_commenting = df_pr_review_commenting.query(
            'event_type == "PullRequestReviewCommentEvent"'
        )  # The PullRequestReviewEvent with review_state as commented
    ...
    df_all_activities = pd.concat(
        [df_all_activities, df_pr_review_commenting.drop_duplicates()[general]]
    )
...
def ReviewingCode(df_events, list_event_id_covered, df_all_activities):
    ...
    df_pr_review_code = (
        df_events[event_general + ["review_node_id"]]
    ...
    df_all_activities = pd.concat(
        [df_all_activities, df_pr_review_code.drop_duplicates()[general]]
    )
...
def CommentingCommits(df_events, list_event_id_covered, df_all_activities):
    ...
    df_all_activities = pd.concat(
        [df_all_activities, df_commit_commenting.drop_duplicates()[general]]
    )
...
def PushingCommits(df_events, list_event_id_covered, df_all_activities):
    ...
    """
    pushing_commits = (
        df_events[event_general + ["push_id"]]
    ...
    df_all_activities = pd.concat(
        [df_all_activities, pushing_commits.drop_duplicates()[general]]
    )
...
def activity_identification(df_events):
    ...
    if df_events.columns.isin(["issue_closed_at"]).any():
        df_events["issue_closed_at"] = pd.to_datetime(
            df_events.issue_closed_at, errors="coerce", format="%Y-%m-%dT%H:%M:%SZ"
        ).dt.tz_localize(None)
    df_events["created_at"] = pd.to_datetime(
        df_events.created_at, errors="coerce", format="%Y-%m-%dT%H:%M:%SZ"
    ).dt.tz_localize(None)
    ...
    if df_events.columns.isin(["ref_type"]).any():
        temp_df = df_events.query(
            'event_type == "CreateEvent" and ref_type == "repository"'
        )
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = CreatingRepository(
                temp_df, list_event_id_covered, df_all_activities
            )

        temp_df = df_events.query(
            'event_type == "CreateEvent" and ref_type == "branch"'
        )
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = CreateBranch(
                temp_df, list_event_id_covered, df_all_activities
            )

        if df_events.columns.isin(["release_node_id"]).any():
            temp_df = df_events.query(
                'event_type == "ReleaseEvent" or (event_type == "CreateEvent" and ref_type == "tag")'
            )
            if temp_df.shape[0] > 0:
                df_all_activities, list_event_id_covered = PublishingRelease(
                    temp_df, list_event_id_covered, df_all_activities
                )
    if (
        df_events.columns.isin(["release_node_id"]).any()
        and ~df_events.columns.isin(["ref_type"]).any()
    ):
    ...
    if df_events.columns.isin(["ref_type"]).any():
        temp_df = df_events.query(
            'event_type == "CreateEvent" and ref_type == "tag" and event_id not in @list_event_id_covered'
        )
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = CreatingTag(
                temp_df, list_event_id_covered, df_all_activities
            )

        temp_df = df_events.query('event_type == "DeleteEvent" and ref_type == "tag"')
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = DeletingTag(
                temp_df, list_event_id_covered, df_all_activities
            )

        temp_df = df_events.query(
            'event_type == "DeleteEvent" and ref_type == "branch"'
        )
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = DeletingBranch(
                temp_df, list_event_id_covered, df_all_activities
            )

    temp_df = df_events.query('event_type == "PublicEvent"')
    if temp_df.shape[0] > 0:
        df_all_activities, list_event_id_covered = MakingRepositoryPublic(
            temp_df, list_event_id_covered, df_all_activities
        )

    temp_df = df_events.query('event_type == "MemberEvent"')
    if temp_df.shape[0] > 0:
        df_all_activities, list_event_id_covered = AddingCollaborator(
            temp_df, list_event_id_covered, df_all_activities
        )

    temp_df = df_events.query('event_type == "ForkEvent"')
    if temp_df.shape[0] > 0:
        df_all_activities, list_event_id_covered = ForkingRepository(
            temp_df, list_event_id_covered, df_all_activities
        )

    temp_df = df_events.query('event_type == "WatchEvent"')
    if temp_df.shape[0] > 0:
        df_all_activities, list_event_id_covered = StarringRepository(
            temp_df, list_event_id_covered, df_all_activities
        )

    temp_df = df_events.query('event_type == "GollumEvent"')
    if temp_df.shape[0] > 0:
        df_all_activities, list_event_id_covered = EditingWikiPage(
            temp_df, list_event_id_covered, df_all_activities
        )

    if df_events.columns.isin(["action"]).any():
        if (
            df_events.columns.isin(["num_comments"]).any()
            and df_events.columns.isin(["issue_closed_at"]).any()
        ):
            temp_df = df_events[
                (df_events["event_type"] == "IssuesEvent")
                & (df_events["action"] == "opened")
                & (
                    (df_events["num_comments"] > 0)
                    | (df_events["issue_closed_at"].notnull())
                )
            ]
            if temp_df.shape[0] > 0:
                df_all_activities, list_event_id_covered = TransferingIssue(
                    temp_df, list_event_id_covered, df_all_activities
                )
        temp_df = df_events.query(
            'event_type == "IssuesEvent" and action == "opened" and event_id not in @list_event_id_covered'
        )
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = OpeningIssue(
                temp_df, list_event_id_covered, df_all_activities
            )

        temp_df = df_events.query(
            '(event_type == "IssuesEvent" and action == "closed") or event_type == "IssueCommentEvent"'
        )
        temp_df = temp_df.dropna(axis=1, how="all")
        if (
            temp_df.shape[0] > 0
            and temp_df.columns.isin(["issue_node_id"]).any()
            and temp_df.columns.isin(["issue_closed_at"]).any()
        ):
            df_all_activities, list_event_id_covered = ClosingIssue(
                temp_df, list_event_id_covered, df_all_activities
            )

        temp_df = df_events.query(
            '((event_type == "IssuesEvent" and action == "reopened") or event_type == "IssueCommentEvent") \
        and event_id not in @list_event_id_covered'
        )
        temp_df = temp_df.dropna(axis=1, how="all")
        if temp_df.shape[0] > 0 and temp_df.columns.isin(["issue_node_id"]).any():
            df_all_activities, list_event_id_covered = ReopeningIssue(
                temp_df, list_event_id_covered, df_all_activities
            )

    temp_df = df_events.query(
        'event_type == "IssueCommentEvent" and event_id not in @list_event_id_covered '
    )
    temp_df = temp_df.dropna(axis=1, how="all")
    if temp_df.columns.isin(["issue_number"]).any():
        temp_df = temp_df[temp_df["issue_number"].notnull()]
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = CommentingIssue(
                temp_df, list_event_id_covered, df_all_activities
            )
    ...
    if df_events.columns.isin(["action"]).any():
        ...
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = OpeningPullRequest(
                temp_df, list_event_id_covered, df_all_activities
            )

        temp_df = df_events.query(
            '((event_type == "PullRequestEvent" and action == "reopened") or event_type == "IssueCommentEvent") \
        and event_id not in @list_event_id_covered'
        )
        temp_df = temp_df.dropna(axis=1, how="all")
        if temp_df.shape[0] > 0 and temp_df.columns.isin(["PR_node_id"]).any():
            df_all_activities, list_event_id_covered = ReopeningPullRequest(
                temp_df, list_event_id_covered, df_all_activities
            )

        temp_df = df_events.query(
            '((event_type == "PullRequestEvent" and action == "closed") or \
        event_type == "PushEvent" or event_type == "IssueCommentEvent") and event_id not in @list_event_id_covered'
        )
        temp_df = temp_df.dropna(axis=1, how="all")
        if (
            temp_df.shape[0] > 0
            and temp_df.columns.isin(["PR_node_id"]).any()
            and temp_df.columns.isin(["merged"]).any()
        ):
            df_all_activities, list_event_id_covered = ClosingPullRequest(
                temp_df, list_event_id_covered, df_all_activities
            )

    if df_events.columns.isin(["PR_number"]).any():
        temp_df = df_events[
            (df_events["event_type"] == "IssueCommentEvent")
            & (df_events["PR_number"].notnull())
            & (~df_events["event_id"].isin(list_event_id_covered))
        ]
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = CommentingPullRequest(
                temp_df, list_event_id_covered, df_all_activities
            )

    if df_events.columns.isin(["review_state"]).any():
        temp_df = df_events.query(
            '(event_type == "PullRequestReviewCommentEvent") or \
        (event_type == "PullRequestReviewEvent" and review_state == "commented")'
        )
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = CommentingPullRequestChanges(
                temp_df, list_event_id_covered, df_all_activities
            )
        ...
        if temp_df.shape[0] > 0:
            df_all_activities, list_event_id_covered = ReviewingCode(
                temp_df, list_event_id_covered, df_all_activities
            )

    temp_df = df_events.query('event_type == "CommitCommentEvent"')
    if temp_df.shape[0] > 0:
        df_all_activities, list_event_id_covered = CommentingCommits(
            temp_df, list_event_id_covered, df_all_activities
        )

    temp_df = df_events.query(
        'event_type == "PushEvent" and event_id not in @list_event_id_covered'
    )
    if temp_df.shape[0] > 0:
        df_all_activities, list_event_id_covered = PushingCommits(
            temp_df, list_event_id_covered, df_all_activities
        )
