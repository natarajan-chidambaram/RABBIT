...
from . import ExtractEvent as eev
from . import GenerateActivities as gat

# import ComputeFeatures as cfe
from . import important_features as imf
...
def get_model():
    ...
    try:
        resource_path = files("rabbit").joinpath(f"data/{model_file}")
...
def check_ratelimit(ratelimit, nextResetTime, max_queries):
    ...
    if ratelimit < max_queries:
        ...
        print(
            "Remaining API query limit is {0}. Querying paused for {1}s until next reset time: {2}".format(
                ratelimit, pause, ResetTime
            )
        )
        time.sleep(pause)
...
def arg_parser():
    parser = argparse.ArgumentParser(
        description="RABBIT is an Activity Based Bot Identification Tool that identifies bots based on their recent activities in GitHub"
    )
    parser.add_argument(
        "contributor",
        action="store",
